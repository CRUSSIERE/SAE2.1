...
void lecteur::chargerDiaporama() {
    ...
    _numDiaporamaCourant = 1; // Initialise le numéro du diaporama courant à 1
    ...
    for (int i = 0; requete.next(); i++) {
        Image *imageACharger = new Image(requete.value(0).toInt(), requete.value(2).toString().toStdString(), requete.value(3).toString().toStdString(), "//haya/dossiersetud_BaieSsd/ggratchev/Downloads" + requete.value(1).toString().toStdString());
...
void lecteur::changerVitesse() {
    ...
    Dialog *changerVitesse = new Dialog(nullptr);
    changerVitesse->exec();
